...
Input - **rstanarm** is able to take a data frame as input[^2].

Missing Data - **rstanarm** automatically discards observations with NA values for any variable used in the model[^3].

Identifiers - **rstanarm** does not require identifiers to be sequential[^4]. We do suggest that it is good practice for all cluster and unit identifiers, as well as categorical variables be stored as factors. This applies to using **lme4** as much as it does to **rstanarm**. One can check the structure of the variables by using the `str()` function.
...
The varying intercept model[^5] with an indicator variable for being female $x_{ij}$ can be written as
...
The BLUPs are equivalent to the so-called Empirical Bayes (EB) prediction, which is the mean of the posterior distribution of $u_j$ given all the estimated parameters, as well as the random variables $y_{ij}$ and $x_{ij}$ for the cluster. These predictions are called "Bayes" because they make use of the pre-specified prior distribution[^6] $\alpha_j \sim N(\mu_\alpha,\sigma^2_\alpha)$, and by extension $u_j \sim N(0,\sigma^2_\alpha)$, and called "Empirical" because the parameters of this prior, $\mu_\alpha$ and $\sigma^2_\alpha$, in addition to $\beta$ and $\sigma^2_y$, are estimated from the data.
...
A more direct approach to obtaining the posterior draws for specific parameters is to make use of the built in functionality of the `as.matrix` method for `stanreg` objects. When applying the `as.matrix` method to a `stanreg` object, the user is able to specify either an optional character vector of parameter names, or an optional character vector of regular expressions[^10] to extract the posterior draws of only the parameters they are interested in. For example, since the parameter representing the overall mean is labelled `(Intercept)`, we can extract the posterior draws of only this parameter by including the option `pars = "(Intercept)"`. Similarly, since the parameters representing the 73 school-level errors all contain the string `b[(Intercept) school:` we can extract all parameters that contain this string by using the option  `regex_pars = "b\\[\\(Intercept\\) school\\:"`.
...
$$
\begin{align}
\Sigma &=  \begin{pmatrix} \sigma^2_\alpha & \rho\sigma_\alpha\sigma_\beta \\ \rho\sigma_\alpha\sigma_\beta & \sigma^2_\beta \end{pmatrix} \\
&= \sigma^2_y \begin{pmatrix} \sigma^2_\alpha/\sigma^2_y & \rho\sigma_\alpha\sigma_\beta/\sigma^2_y \\ \rho\sigma_\alpha\sigma_\beta/\sigma^2_y & \sigma^2_\beta/\sigma^2_y \end{pmatrix} \\
&= \sigma^2_y
\begin{pmatrix} \sigma_\alpha/\sigma_y & 0 \\ 0 & \sigma_\beta/\sigma_y \end{pmatrix} 
...
The correlation matrix $R$ is 2 by 2 matrix with 1’s on the diagonal and $\rho$’s on the off-diagonal. `stan_lmer` assigns it an LKJ[^11] prior (Lewandowski, Kurowicka, and Joe 2009), with regularization parameter 1. This is equivalent to assigning a uniform prior for $\rho$. The more the regularization parameter exceeds one, the more peaked the distribution for $\rho$ to take the value 0.
...
```{r rstan convergence-01}
plot(M1_stanlmer, "rhat")
```

The $\text{N}_\text{eff}$ statistic represents the effective number of simulation draws. If the draws were independent $\text{N}_\text{eff}$ would be the number of saved draws, 4,000 (4 chains × (2,000 iterations - 1,000 iterations for warmup)), but $\text{N}_\text{eff}$ tends to be smaller than 4,000 because Markov chain simulations tend to be autocorrelated. The $\text{N}_\text{eff}$ statistic should typically be at least 100 across parameters. We can plot a histogram of the ratio of effective sample size to total sample size:

```{r rstan convergence-02}
...
[^5]: Equivalently, the model can be expressed using a two-stage formulation as
$$\begin{align} y_{ij} &= \alpha_j+\beta x_{ij} + \epsilon_{ij}, \\
\alpha_j &= \mu_\alpha+u_j \end{align},$$
or in a reduced form as
$$y_{ij}=\mu_\alpha+\beta x_{ij}+u_j+\epsilon_{ij}$$
where $\epsilon_{ij} \sim N(0,\sigma^2_y)$ and $u_j \sim N(0,\sigma^2_\alpha)$.

[^6]: We elaborate more on prior distributions in Section 3.2

[^7]: For users who are not familiar with **ggplot2** syntax, we refer them to [here](https://www.rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf).

[^8]: Equivalently, the model can be expressed in a two-stage formulation as
$$\begin{align} y_{ij} &= \alpha_j+\beta_j x_{ij} + \epsilon_{ij}, \\
\alpha_j &= \mu_\alpha + u_j, \\
\beta_j &= \mu_\beta + v_j\end{align},$$
or in a reduced form as $y_{ij} = \mu_\alpha+\mu_\beta x_{ij}+u_j+v_j x_{ij}+\epsilon_{ij}$
where $\epsilon_{ij} \sim N(0,\sigma^2_y)$ and $\begin{pmatrix} u_j \\ v_j \end{pmatrix} \sim N \left( \begin{pmatrix} 0 \\ 0 \end{pmatrix},\begin{pmatrix} \sigma^2_\alpha & \rho\sigma_\alpha\sigma_\beta \\ \rho\sigma_\alpha\sigma_\beta & \sigma^2_\beta \end{pmatrix} \right)$.
...
[^10]: For more information on regular expressions, see [here](http://www.rstudio.com/wp-content/uploads/2016/09/RegExCheatsheet.pdf)

[^11]: For more details about the LKJ distribution, see [here](http://www.psychstatistics.com/2014/12/27/d-lkj-priors/) and [here](http://mc-stan.org/users/documentation/case-studies/lotka-volterra-predator-prey.html)

[^12]: The Dirichlet distribution is a multivariate generalization of the beta distribution with one concentration parameter, which can be interpreted as prior counts of a multinomial random variable (the simplex vector in our context), for details, see [here](https://cran.r-project.org/web/packages/rstanarm/vignettes/glmer.html#details).